...
const Benefits = () => {
  return (
    <section id="benefits" className="pt-6 pb-16 lg:pt-8 lg:pb-24 bg-gradient-to-b from-background via-muted/20 to-background relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5 lg:gap-6">
          {benefits.map((benefit, index) => {
            ...
            return (
              <div
                key={index}
                className="group text-center p-4 sm:p-6 lg:p-8 rounded-2xl lg:rounded-3xl bg-white/80 backdrop-blur-sm hover:bg-white border border-border/50 hover:border-primary/30 hover:shadow-float transition-all duration-500 hover:-translate-y-2 relative overflow-hidden"
              >
                <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <div className="relative">
                  <div className="inline-flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 rounded-2xl bg-gradient-to-br from-[#c5162a] to-[#e63946] mb-4 lg:mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 shadow-glow">
                    <Icon className="w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-white" />
                  </div>
                  <h3 className="text-sm sm:text-lg lg:text-xl font-extrabold text-foreground mb-2 lg:mb-3 group-hover:text-primary transition-colors duration-300">
                    {benefit.title}
                  </h3>
                  <p className="text-xs sm:text-sm lg:text-base text-muted-foreground leading-relaxed font-medium">
                    {benefit.description}
                  </p>
                </div>
              </div>
